...
def process_xml_field_removal(xml_content, field_path, modification_mode="comment"):
    ...
    Supports nested paths and namespaces.
    """
    try:
        # Parse XML
        root = ET.fromstring(xml_content)

        # Split the field_path into parts, handle namespaces
        path_parts = [part for part in field_path.strip('/').split('/') if part]

        # Helper to recursively find parent and target
        def find_parent(element, path_parts):
            if len(path_parts) == 1:
                # Return parent and target element
                for child in element:
                    # Remove namespace for matching
                    tag_no_ns = child.tag.split('}', 1)[-1] if '}' in child.tag else child.tag
                    if tag_no_ns == path_parts[0]:
                ...
                for child in element:
                    tag_no_ns = child.tag.split('}', 1)[-1] if '}' in child.tag else child.tag
                    if tag_no_ns == path_parts[0]:
                        return find_parent(child, path_parts[1:])
                return None, None

        parent, target = find_parent(root, path_parts)
        if parent is not None and target is not None:
            if modification_mode == "remove":
                parent.remove(target)
            elif modification_mode == "comment":
                comment_text = f" commented-{target.tag}: {target.text} "
                parent.insert(list(parent).index(target), ET.Comment(comment_text))
                parent.remove(target)
            # Output new XML string
            return ET.tostring(root, encoding='unicode')
        else:
            print(f"⚠️ Could not find field in XML: {field_path}")
            return xml_content
